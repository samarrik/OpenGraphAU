...
from . import swin_transformer as swin_mod
...
class MEFARG(nn.Module):
    def __init__(self, num_main_classes = 27, num_sub_classes = 14, backbone='swin_transformer_base', neighbor_num=4, metric='dots', pretrain_dir: str | None = None):
        super(MEFARG, self).__init__()
        if pretrain_dir:
            try:
                resnet_mod.set_pretrain_dir(pretrain_dir)
            except Exception:
                pass
            try:
                swin_mod.set_pretrain_dir(pretrain_dir)
            except Exception:
                pass
    ...
    def forward(self, x):
        # Accept single image tensor (3, H, W) by adding batch dim
        if x.dim() == 3:
            x = x.unsqueeze(0)
